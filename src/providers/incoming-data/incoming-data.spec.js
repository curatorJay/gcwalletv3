...
describe('Provider: Incoming Data Provider', () => {
    ...
    describe('Function: SCANNER Redir', () => {
        ...
        it('Should handle Plain URL', fakeAsync(() => {
            spyOn(payproProvider, 'getPayProDetails').and.returnValue(Promise.reject(true));
            let data = [
                'http://getcoins.com/',
        ...
        it('Should handle Join Wallet', () => {
            let data = 'getcoins:RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc';
        ...
        it('Should handle QR Code Export feature', () => {
            ...
            data.forEach(element => {
                ...
                expect(incomingDataProvider.redir(element, 'ScanPage')).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Handling QR Code Export feature');
                expect(eventsSpy).toHaveBeenCalledWith('IncomingDataRedir', nextView);
            });
        });
        it('Should handle BTC and BCH BitPay Invoices', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, 'ScanPage')).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Handling Payment Protocol with non-backwards-compatible request');
            });
        });
        it('Should handle Bitcoin cash Copay/BitPay format and CashAddr format plain Address', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, 'ScanPage')).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Handling Bitcoin Cash Plain Address');
                expect(eventsSpy).toHaveBeenCalledWith('showIncomingDataMenuEvent', {
                    data: element,
                    type: 'bitcoinAddress',
                    coin: 'bch'
        ...
        it('Should Handle Bitcoin Cash URI with legacy address', fakeAsync(() => {
            ...
            expect(incomingDataProvider.redir(data, 'ScanPage')).toBe(true);
            expect(loggerSpy).toHaveBeenCalledWith('Handling Bitcoin Cash URI with legacy address');
        ...
        it('Should handle Bitcoin Livenet and Testnet Plain Address', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, 'ScanPage')).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Handling Bitcoin Plain Address');
                expect(eventsSpy).toHaveBeenCalledWith('showIncomingDataMenuEvent', {
                    data: element,
                    type: 'bitcoinAddress',
                    coin: 'btc'
